...
lm_fit <- lm_model |> fit(Quality.of.Sleep ~ ., data = train_ready)

lm_fit |> extract_fit_engine() |> summary()
```

```{r}
pred_reg <- predict(lm_fit, new_data = train_ready)
...
```{r}
rmse(compare, Quality.of.Sleep, .pred)
```
...
pred_tree3 <- predict(forest_fit, new_data = train_ready)

compare3 <- train_ready |> 
  select(Quality.of.Sleep) |> 
...
```{r}
rmse(compare3, Quality.of.Sleep, .pred)
```
