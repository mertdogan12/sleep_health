...
```{r}
forest_rec <- recipe(Quality.of.Sleep ~ ., data = final_sleep_data_train) |>
  step_impute_median(all_numeric_predictors()) |>
  step_dummy(all_nominal_predictors()) |>
  step_normalize(all_predictors()) 
...
forest_param <- forest_wflow %>% 
  ...
  finalize(final_sleep_data_train)

forest_folds <- vfold_cv(final_sleep_data_train)
...
select_best(forest_tune, metric = "rmse")
```
...
final_forest_wflow <- forest_wflow %>%
  finalize_workflow(final_forest_param)

final_forest_wflow

final_forest_fit <- final_forest_wflow %>%
  fit(final_sleep_data_train)
```
...
```{r}
pred_test <- predict(final_forest_fit, new_data = final_sleep_data_test)
