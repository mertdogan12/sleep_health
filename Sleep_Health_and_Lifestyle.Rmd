...
# Wie wirkt sich Stress und Körperliche Aktivität auf die Schlaf Qualität aus?
...
Schauen wir uns erstmal an wie sich Stress auf die Schlafqualität auswirkt. Dabei wird die Variable *Stress.Level* mit der Variable *Quality.of.Sleep* dem Diaram verglichen.
...
```
- AUsreizer entfernen
- Schlafqualität nimmt eindeutig mit mehr Stress ab


```{r}
final_sleep_data %>%
  ggplot(mapping = aes(x = Stress.Level, fill = Sleep.Disorder == "Has sleep disorder")) +
  geom_bar() +
  ...
  labs(
    ...
    x = "Stress Level (1 - 10)",
    y = "Anzahl Personen",
    fill = "Hat einen Schlafstörung?"
  )
```

- Schl

Wie man an dem Diagram erkennen kann nimmt Schlafqulität mit steigenden Stresslevel eindeutig ab.
...
- Wenig Stress wieder eindeutig gute schlafqualität
- 0 schritte geht nicht
- Viele Körperliche Aktivität/Viele Schritte --> Stress --> Schlecher schlaf
- Gewisses maß an Sport --> weniger Stress + bessere Schlafqualität
