...
final_sleep_data %>%
  select(Quality.of.Sleep, Physical.Activity.Level.Percent, Daily.Steps.Percent) %>%
  pivot_longer(
  ...
  ggplot(
    data = .,
    mapping = aes(x = Value, y = Quality.of.Sleep, colour = Variable)
  ) +
  geom_smooth() +
  labs(
    title = "Körperliche Aktivität und Schlafqualität",
    x = "Prozent", y = "Schlafqualität"
  )
```
