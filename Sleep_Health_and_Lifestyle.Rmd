...
Die Grafik zeigt, dass die Schlafqulität mit zunehmeden Stress abnimmt.
...
Bis zu einem Stresslevel von 6 bleibt die Schlafqulität also über dem Durschnitt. Somit also relativ zum Datensatz gesehen im höheren Bereich. Ab einen Stresslevel von 6 fällt die Schlafqulität darunter.

Dazu muss man aber sagen, dass die Daten subjektiv sind. Um eine genauers Bild zu bekommen schaun wird uns an wie sich Stress auf die Schlafstörungen auswirk. Dazu betrachen wir, wie groß der Teil an Personen sind, die an Schlafstörungen leiden.
...
Wie wird sehen können leigt der Schlafstörungsanteil bei Stresslevel 1 bis 6 unter 50 Prozent. Man kann sogar einen Abfall der Schlafstörungen, bis zu einem Stresslevel von 6 erkennen . Nach einem Stresslevel von 6 hat man aber einen starken Anstiegt von Schlafstörungen. Somit kann man sagen, dass trotz dem Abfall am Anfgang, Stress die Schlafsörungen deutlich erhöht. Somit bestäigen wir unsere vorherigen Ergebnisse.

Nun schauen wir uns an wie sich Körperliche Aktivität auf die Schlafqulität auswirkt.
...
Wie man sehen kann wirkt sich die Körperliche Aktivität nicht so eindeutig auf die Schlafqualität aus. Man kann aber einen leichten anstiegt erkenne. Es lassen sich zudem, zwei größere Außreizer erkenn. Einen bei einer guten Schlafqulität trotz Körperlicher AKtivität und eien bei schlechter Schlafqulität trotz hoher Körperlicher Aktivität. Diese habe aber ein hohes und niedriges Stresslevel. Die Schlafqulität ist also stark von Stresslevel beeinflusst.
...
Um ein noch genaueres Bild zu erhalten schauen wird uns die verteilung der Schlafstörungen auf die Körperliche Aktivität an.
...
Zudem vergleichen wir auch ob die Anzahl der Täglischen Schritte einen Zusammenhang mit Schlafstörungen hat.
...
Ähnliche wie im Diagram *Schlafstörungen und Körperliche Aktivität* haben die Personen ohne Schlafstörung eine hohen Spannweite und einen höheren Median als Personen ohne Schlafstörung.

Dagegen haben die Personen mit einer Schlafstörungen einen niedrige Spannweite Der Median ist zwar niedriger, dafür ist das unter Quatil höher.

Somit ähnelt sich das Diagram Stark mit dem Diagram *Schlafstörungen und Körperliche Aktivität*. Nur das obere Quartil der Personen mit einer Schlaftörung fältt relativ zum vorherigen Diagram niedriger aus. Trotzdem kann man sagen, dass Täglische Schritte keine eindeutigen zusammenhand zu den vorkommen von Schlafstörungen hat.

```{r}
final_sleep_data %>%
  select(Quality.of.Sleep, Physical.Activity.Level, Daily.Steps, Stress.Level) %>%
  cor() %>%
  corrplot(method = 'ellipse')
```

- Stress und Herzfreq. hohe Korrelation mit Schlaf
- Sport leichte Korrelation mit Stress
- und sehr leiche Korrelation mit Schlaf
...
*Wann hat man die beste Schlafqualität*

*Noch nimt Schlafstörungen*